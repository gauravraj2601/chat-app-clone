...
const MessageBox = ({chats}) => {
  ...
  return (  
    <>
    {chats && chats?.map((eachChat)=>(
      // console.log(eachChat?.users)
    <div onClick={()=>setSelectedChat(eachChat)} className={`w-[317px] h-[93px] m-auto flex justify-center items-center mt-3 rounded-2xl ${selectedChat === eachChat ? "SelectedChat-bg cursor-pointer" : "chat-bg"}  `}>
      ...
      <div className="w-[229px] h-[69px] flex flex-col gap-2 ">
        <div className="h-[39px] flex flex-col  ">
          <div className="h-[21px] flex gap-3 text-[14px] font-inter ">
            <div className="w-[186px]  ">{!eachChat.isGroupChat ? getSender(user,eachChat?.users): eachChat?.chatName} </div>
        ...
        <div className="h-[22px] w-[97px] rounded-2xl px-0.5 py-2 border border-[#CBD5E0] flex justify-center items-center "> <p className="font-inter text-xs text-[ #CBD5E0] " >Some Content</p> </div>
      </div>
    </div>
 </div>
    ))}
